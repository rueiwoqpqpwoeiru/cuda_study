...
class Color2gray {
public:
	void do_process(const Process_size& p, const Device_memory<uchar3>& in, Device_memory<uchar3>& out) {
		Kernel_func::generic_kernel<Color2gray> << < p.grid(), p.block() >> > (this, in.data_for_kernel(), out.data_for_kernel());
	}
	// 関数名はkernelに固定
	__device__ __forceinline__ void kernel(const Data_for_kernel<uchar3> in, Data_for_kernel<uchar3> out) {
...
int main(int argc, char* argv[]) {
	...
	for (unsigned int i = 0; i < 1; i++) {
		...
		color2gray.do_process(process_size, in_img, out_img);
